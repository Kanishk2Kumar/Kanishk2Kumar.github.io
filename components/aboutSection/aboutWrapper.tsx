...
export function AboutWrapper({}) {
  const [text, setText] = useState({
    main: "Featured Work",
    para: `I build high-end, pixel-perfect websites and production-ready systems that can scale with your business. Blending great user experience with fast, reliable, cloud-powered backends.`,
  });
  useEffect(() => {
    if (!isDesktop()) {
      setText({
  ...
  return (
    <main className="flex h-full w-full max-w-maxWidth grow flex-col justify-center text-[5.8vw] md:text-[clamp(20px,_1vw_+_14px,_32px)]  ">
      <div className="anime relative flex flex-col gap-[1em] md:flex-row-reverse md:gap-[2em] ">
        ...
        >
          <p className="shapka !flex text-[0.9em]  text-colorLight md:text-[0.8em]">
            <span className="scrambleText whitespace-nowrap">
              View all Work
            </span>
            <svg
              className="ml-8 w-[0.75em] -rotate-45 text-colorLight  " // width="34px"
              // height="34px"
              viewBox="0 0 14 14"
              version="1.1"
